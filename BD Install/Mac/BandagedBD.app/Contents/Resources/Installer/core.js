...
function install(appPath, log, error, progress, restart) {
    ...
    try {
        ...
        .then(() => {
            ...
            const checkFiles = ["index.js", "package.json", "betterdiscord/index.js", "betterdiscord/preload.js", "betterdiscord/config.json", "betterdiscord/logger.js"];
            let verified = true;
            for (let file in checkFiles) {
                log(`Checking ${checkFiles[file]}...`, true);
                if (!fs.existsSync('./app/' + checkFiles[file])) {
                    err('Missing file: ' + checkFiles[file] + '! -- Please retry install.');
                    verified = false;
                }
            }
            if (restart && verified) {
                log('All files verified!', true);
                progress(90);
